...
public class SaveCookie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // 쿠키 만들기(쿠키 이름, 유지 시간, 저장 경로)              쿠키는 각 클라이언트의 개인pc에 저장됨
	  // 쿠키 이름과 값
	  ...
	  // 쿠키 저장 경로 (생략하면 contextPath가 경로로 사용된다. 그래서 생략가능)
	  cookie1.setPath("/servlet");           // contextPath ,   cookie1.setPath(request.getContextPath())
	  cookie2.setPath("/servlet/saveCookie"); // URLMapping,     cookie2.setPath(request.getRequestURI()) // 이렇게 쿠키의 저장경로를 지정해놓으면  /servlet 에서는 쿠키가 사라진다. 특정 페이지에서 쿠키가 필요할때!
	  ...
	  response.sendRedirect(request.getContextPath() + "/readCookie"); //getContextPath()를 변수처리하는 건 꽤 중요함. 배포할 떄 getContextPath 이게 없을 수도 있음...다 뜯어고칠 때 변수처리하면 편하기 때문
